...
// -- 	การลาแต่ละครั้ง
// CASE 
//  WHEN  DATE(NOW()) <=  ('2021-02-28') AND (employee.Empstatus_ID=201 OR employee.Empstatus_ID=202 OR employee.Empstatus_ID=203)
// THEN
// IFNULL(sum(`leave`.LeaveTotal),0) 
// WHEN  DATE(NOW()) >=  ('2021-02-28') AND (employee.Empstatus_ID=201 OR employee.Empstatus_ID=202 OR employee.Empstatus_ID=203)
// THEN
// 0
// WHEN  DATE(NOW()) >=  ('2021-02-28') 
// AND (employee.Empstatus_ID=201 OR employee.Empstatus_ID=202 OR employee.Empstatus_ID=203)
// THEN
// IFNULL(sum(`leave`.LeaveTotal),0) 
...
// 			WHEN   DATE(NOW()) <=  ('2021-02-28')  AND employee.Empstatus_ID=201  
// 			THEN  	
...
//    WHEN  DATE(NOW()) <=  ('2021-02-28')  AND (employee.Empstatus_ID=202 OR employee.Empstatus_ID=203)  AND
//        DATEDIFF(NOW(),employee.Work_day ) / 365 < 10   
// 		 AND COUNT(`leave`.LType_ID) <1
// 		 THEN  
//        IFNULL(leavetype.Number,leavetype.Number)
         
//              WHEN  DATE(NOW()) >=  ('2021-02-28')  AND (employee.Empstatus_ID=202 OR employee.Empstatus_ID=203)  AND
//        DATEDIFF(NOW(),employee.Work_day ) / 365 < 10   
// 		 AND COUNT(`leave`.LType_ID) <1
// 		 THEN  
//        IFNULL(leavetype.Number,leavetype.Number) +leavetype.Number
         
//           -- ลาพักผ่อน วันปัจจุบันน้อยกว่าวันปีไหม่ แล่อายุงานไม่ถึง 10 ปี และ วันลาสะสมไม่ถึง 20 วัน จะคิดคำนวนปกติ

		
// 		 WHEN  DATE(NOW()) <  ('2021-02-28')  AND (employee.Empstatus_ID=202 OR employee.Empstatus_ID=203)  AND
//        DATEDIFF(NOW(),employee.Work_day ) / 365 < 10  AND IFNULL(leavetype.Number -  sum(`leave`.LeaveTotal),leavetype.Number)+leavetype.Number < 20
// 		 THEN  
//          IFNULL(leavetype.Number -  sum(`leave`.LeaveTotal),leavetype.Number) 
...
//     WHEN   DATE(NOW()) <= ('2021-02-28')  AND (employee.Empstatus_ID=202 OR employee.Empstatus_ID=203)  AND
//        	DATEDIFF(NOW(),employee.Work_day ) / 365 > 10 AND IFNULL(leavetype.Number -  sum(`leave`.LeaveTotal),leavetype.Number)+leavetype.Number < 30
// 		 THEN  
//          IFNULL(leavetype.Number -  sum(`leave`.LeaveTotal),leavetype.Number)
