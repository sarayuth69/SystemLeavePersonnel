...
IFNULL(sum(`leave`.number_leave),0) AS number_leave_show,
  ...
  WHERE 
    ...
    AND leavetype.LType_ID != 4003 AND leavetype.LType_ID != 4040 AND leavetype.LType_ID = '".$_POST["LType_ID"]."'
  GROUP BY
  employee.Emp_ID,
  `leave`.limit_ID,
  leavetype.LType_ID
...
IFNULL(sum(`leave`.number_leave),0) AS number_leave_show,
  ...
  WHERE 
    employee.Emp_ID ='".$_POST["Emp_ID"]."' AND leavetype.QuotaStatus = 'ลาได้'AND leavetype.LTypeName != 'ลาพักผ่อน'
    AND leavetype.LType_ID != 4038 AND leavetype.LType_ID != 4039 AND leavetype.LType_ID = '".$_POST["LType_ID"]."'
  GROUP BY
  employee.Emp_ID,
  `leave`.limit_ID,
  leavetype.LType_ID
...
IFNULL(sum(`leave`.number_leave),0) AS number_leave_show,
  ...
  WHERE 
    employee.Emp_ID ='".$_POST["Emp_ID"]."' AND leavetype.QuotaStatus = 'ลาได้'AND leavetype.LTypeName != 'ลาพักผ่อน'
    AND leavetype.LType_ID = '".$_POST["LType_ID"]."'
  GROUP BY
  employee.Emp_ID,
  `leave`.limit_ID,
  leavetype.LType_ID
